...
def time_passed(sort_func, data):
...
# вбудованt сортування Python
def built_in_sort(arr : list):
    arr.sort()
    return arr

# Генеруємо випадкові дані для тестування
data_smallest = [random.randint(0, 1_00) for _ in range(10)]
data_small = [random.randint(0, 1_000) for _ in range(100)]
data_big = [random.randint(0, 5_000) for _ in range(1_000)]
data_hudge = [random.randint(0, 10_000) for _ in range(10_000)]

test_data = [
    ("Smallest (10)", data_smallest),
    ("Small (100)", data_small),
    ("Big (1,000)", data_big),
    ("Hudge (10,000)", data_hudge)
]
...
sorting_functions = [
    ...
    ("Timsort (list.sort)", built_in_sort)
]
...
def main():
    ...
    for size_name, data in test_data:
        row = [size_name]
        for _, func in sorting_functions:
            _, exec_time = time_passed(func, data)
